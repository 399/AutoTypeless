...
public struct TypelessController: TypelessControlling {
    ...
    public init(
        bundleIdentifier: String = "now.typeless.desktop",
        applicationURL: URL = URL(fileURLWithPath: "/Applications/Typeless.app", isDirectory: true)
    ) {
    ...
    public func launchNormally() throws {
        try launch(arguments: [])
    }

    private func launch(arguments: [String]) throws {
        guard FileManager.default.fileExists(atPath: applicationURL.path) else {
            throw AutoTypelessError.commandFailed("未找到 Typeless 应用：\(applicationURL.path)")
        }

        // 必须按 macOS App Bundle 方式启动。直接执行 Contents/MacOS/Typeless 虽能传入
        // Electron 调试参数，但会绕过 LaunchServices 的正常应用启动上下文，Typeless 会把
        // 本次启动误判为初始化流程，表现为反复出现“使用引导”。
        ...
        guard semaphore.wait(timeout: .now() + 15) == .success else {
            throw AutoTypelessError.commandFailed("启动 Typeless 超时")
        }
        let launchResult = result.load()
        if let error = launchResult.error {
            throw AutoTypelessError.commandFailed("无法启动 Typeless：\(error.localizedDescription)")
        }
        guard launchResult.application != nil else {
            throw AutoTypelessError.commandFailed("无法启动 Typeless")
        }
    }
